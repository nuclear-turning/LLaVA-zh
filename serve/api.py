...
def load_model(model_path, num_gpus):
    ...
    if 'llava' in model_path.lower():
        from transformers import ChineseCLIPImageProcessor, ChineseCLIPVisionModel
        image_processor = ChineseCLIPImageProcessor.from_pretrained(model.config.mm_vision_tower, torch_dtype=torch.float16)
...
class ModelWorker:
    def __init__(self,
        ...
        logger.info(f"Loading the model llava_zh on worker {worker_id} ...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--multi-modal", default=True,action="store_true", help="Multimodal mode is automatically detected with model name, please make sure `llava` is included in the model path.")
    ...
    model_path = "/home/gpuall/hehx/models/llava-zh-13b-finetune"
    worker = ModelWorker(
                         model_path,
                         args.keep_aspect_ratio,
                         args.num_gpus)
    uvicorn.run(app, host="0.0.0.0", port=21002, log_level="info")
