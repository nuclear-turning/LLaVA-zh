...
class LlavaLlamaModel(LlamaModel):
    ...
    def initialize_vision_modules(self, vision_tower, mm_vision_select_layer,
        ...
        image_processor = ChineseCLIPImageProcessor.from_pretrained(vision_tower)

        if not hasattr(self, 'vision_tower'):
            vision_tower = ChineseCLIPVisionModel.from_pretrained(vision_tower)
        else:
...
class LlavaLlamaForCausalLM(LlamaForCausalLM):
    ...
    def initialize_vision_modules(self, vision_tower, mm_vision_select_layer,
        ...
        if not hasattr(self, 'vision_tower'):
            ...
            vision_tower = self.model.vision_tower[0]
        vision_tower.requires_grad_(False)
        vision_tower = vision_tower.to(torch.float16)
        ...
        if not hasattr(self, 'mm_projector'):
            self.model.mm_projector = nn.Linear(vision_config.hidden_size, self.config.hidden_size)
